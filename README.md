...
```bash
# 1. wait ~15 min, then check
./status.sh                 # ● UP + both ranks running
curl -s -H "Authorization: Bearer $API_KEY" http://127.0.0.1:8000/v1/models
```

**Only if the check fails, start them manually:**
...
> `docker run`), so it always costs a full ~12-minute weight reload. **Do not run it if the
> containers are already up** — it turns a healthy service into a 12-minute outage. Check
> first, and only run it when something is actually down.

#### Decision table

| Symptom after reboot | Action |
|---|---|
| Both containers `Up`, `/v1/models` returns 200 | Nothing. Wait for weight loading to finish. |
| Containers up, still loading (`Loading safetensors…`) | Nothing. ~15 min total. |
